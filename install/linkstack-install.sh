...
$STD apt-get install -y software-properties-common ca-certificates lsb-release apt-transport-https
...
msg_info "Installing LinkStack Dependencies"
...
# Get version from GitHub redirect
LINKSTACK_VERSION=$(curl -sIL "$ZIP_URL" | grep -i location: | grep -oP 'releases/tag/v\K[0-9.]+' | head -n 1 || echo "unknown")
